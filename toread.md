...
## I'm going to show you how to work with data sources on the web, specifically those accessible via an API. Don't worry about what an API is - for our purposes, it's a way to get data from the web. 

# Some background
## Requests to a web resource are organized around verbs: GET to get data, POST to submit data, and more for various other purposes.
...
## We can use the default search method to get data, setting query to 'science', rows to 1 to 
## return only 1 result, and say we want json formatted data with the 'wt' parameter
...
We can easily extract the data with a single function that guesses the type of data, here json
